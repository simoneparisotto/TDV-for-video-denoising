function [u,timecpu,PSNR_TDV] = TDVtime_orders(u,uorig,params)

[M,N,T] = size(u);

%% LOCATION OF GRID VERTEX AND CELL CENTRES
[location.ii_v,       location.jj_v,        location.kk_v]         = ndgrid( 1.5:(M-0.5),  1.5:(N-0.5),  1.5:(T-0.5));
[location.ii_v_extra, location.jj_v_extra,  location.kk_v_extra]   = ndgrid( 0.5:(M+0.5),  0.5:(N+0.5),  0.5:(T+0.5));

[location.ii_u_extra, location.jj_u_extra,  location.kk_u_extra]   = ndgrid(   0:M+1,        0:N+1,        0:T+1 );
[location.ii_du_extra,location.jj_du_extra, location.kk_du_extra]  = ndgrid( 0.5:(M+1.5),  0.5:(N+1.5),  0.5:T+1.5 );
[location.ii_u1_extra,location.jj_u1_extra, location.kk_u1_extra]  = ndgrid( 0.5:(M+1.5),    0:N+1,        0:T+1 );
[location.ii_u2_extra,location.jj_u2_extra, location.kk_u2_extra]  = ndgrid(   0:M+1,      0.5:(N+1.5),    0:T+1 );
[location.ii_u3_extra,location.jj_u3_extra, location.kk_u3_extra]  = ndgrid(   0:M+1,        0:N+1,       0.5:T+1.5 );

location.ii_v_stream  = location.ii_v([1:12:end end],[1:12:end end]);
location.jj_v_stream  = location.jj_v([1:12:end end],[1:12:end end]);
location.kk_v_stream  = location.kk_v([1:12:end end],[1:12:end end]);

%% MIRROR BOUNADRY
u  = padarray(u,[1 1,1],'replicate','both');

%% COMPUTE FIELDS via structure tensor
[V,~,aniso] = compute_structure_Tensor_sliced(u,params.sigma,params.rho,location);

%% plot vector fields for checking purposes
%plot_for_checking
%pause(1)
...
[LR,I] = build_M(V,aniso,params.time_gain,params.varying);

% %% Compute the transfer operators W
% NB the transfer operator to move the derivative of v back to
% cell centres is not yet implemented
w = compute_average_order(M+2,N+2,T+2); % move D^Q on cell centres
%wd = speye((M+2)*(N+2)*(T+2),(M+2)*(N+2)*(T+2)); % keep on D^k u position at k-th derivative
%wc = speye((M-1)*(M-1)*(T-1),(M-1)*(N-1)*(T-1)); % keep on cell centres
Wcal{1} = {w{1}};
%Wcal{2} = {{wd,wd,wd,wd,wd,wd},w{2}};
%Wcal{3} = {{wd,wd},{wd,wd,wd,wd},w{3}};

% %% Compute the weighted derivatives
Mcal{1}  = {LR};
%Mcal{2}  = {I,LR};
%Mcal{3}  = {I,I,LR};
clear V aniso LR I w location

Kcal{1} = MD_orders(u,Mcal{1},Wcal{1},D{1},D{2},D{3});
%Kcal{2} = MD_orders(u,Mcal{2},Wcal{2},D{1},D{2},D{3});
%Kcal{3} = MD(u,Mcal{3},Wcal{3},D1,D2,location);

% Operators
%K         = @(u,params) Ku(Kcal,u,params);
%KS        = @(y,q)      divM(y,Mcal{q},D{1},D{2},D{3},Wcal{q});

%Kfun    = @(u) cat(4,Kfun1(u),Kfun2(u));
%KSfun   = @(y) divM(y,Mcal{1},D{1},D{2},D{3},w);

% MORE COMPACT ALTERNATIVE
% B = compute_average_order2(M+2,N+2,T+2); % move D^Q on cell centres
% Mcal{1} = build_M2(V,params.b,aniso,params.time_gain,params.varying);
% Kcal    = MD2(u,Mcal,B,K{1},K{2},K{3},location);
% Kfun    = @(u) reshape(cat(2,Kcal{1}*u(:),Kcal{2}*u(:),Kcal{3}*u(:)),M-1,N-1,T-1,[]);
% KSfun   = @(y) divM2(y,Mcal{1},K{1},K{2},K{3},B);

%% COMPUTE SADDLE-POINT OPERATORS AND PROX
% F:= (lamdba/eta)*TDV  is the vectorial soft thresholding.
K         = @(u,params)       Ku(Kcal,u,params);
KS        = @(y,q)            divMq(y,Mcal{q},D{1},D{2},D{3},Wcal{q});
